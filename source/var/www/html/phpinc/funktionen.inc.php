  ...
  **************************************************************************/
  function influx_local($daten)   {
    ...
    // if (1 == 1) {
      ...
      $query .= "Wert=".$daten["WattstundenGesamtHeute"].",Wochentag=\"".$daten['Wochentag']."\"  ".$daten["zentralerTimestamp"];
    ...
    $Sonnenaufgang = date_sunrise($now, SUNFUNCS_RET_TIMESTAMP, 50.1143999, 8.6585178, $zenith, $gmt_offset);
      ...
      switch($daten["Regler"]) {
        ...
        case 23:
          ...
          // --------------------------
          if ($daten["SonoffModul"] == 0 or $daten["SonoffModul"] == 43 or $daten["SonoffModul"] == 55) { // POW R2 + Gosung SP1   
            $query .= "AC ";
          ...
          if ($daten["SonoffModul"] == 0) { // Alles unbekannte
            $query .= ",Temperatur=".$daten["Temperatur"];
            $query .= ",Powerstatus1=".$daten["Powerstatus0"];
            $query .= ",Powerstatus2=".$daten["Powerstatus1"];
          ...
          $query .= "\n";
          // --------------------------
          if ($daten["SonoffModul"] == 43 or $daten["SonoffModul"] == 55 or $daten["SonoffModul"] == 200 or $daten["SonoffModul"] == 0) {
            $query .= "Summen ";
            $query .= "Wh_Heute=".$daten["WattstundenGesamtHeute"];
            $query .= ",Wh_Gesamt=".$daten["WattstundenGesamt"];
            $query .= "  ".$daten["zentralerTimestamp"];
  ...
  function sdm_auslesen($USB,$Input) {
    ...
    ************************/
    ...
    if ($address == $Input["DeviceID"] and  $functioncode ==$Input["BefehlFunctionCode"]) {
      return round($this->hex2float($data),2);
    }
    return false;
  }
  ...
  ******************************************************************/
  ...
  function alpha_auslesen($Device,$Befehl='') {
    ...
    do {
      ...
      // $Antwort = fgets($Device,2048);   // geändert 18.12.2020

      //echo bin2hex($Antwort)."\n";

      // if (trim($Antwort) == "") {  geändert 18.12.2020

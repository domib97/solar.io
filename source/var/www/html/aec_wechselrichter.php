  ...
  if (file_exists($Pfad."/../pipe/".$GeraeteNummer.".befehl.steuerung")) {
    ...
    for ($i = 0; $i < count($Befehle); $i++) {
      ...
      *********************************************************************************/
      ...
      // +++ $Wert = $funktionen->aec_inverter_lesen($USB,"#".$InverterAdresse."L ".str_pad(substr($Befehle[$i],1), 3, '0', STR_PAD_LEFT ));
      $Wert = $funktionen->aec_inverter_lesen($USB1,"#".$InverterAdresse.$Befehle[$i]);

      if ($Wert === false) {
        // +++ 20200605 jorcor
        // +++ $funktionen->log_schreiben("Befehlsausführung erfolglos! #".$InverterAdresse."L ".str_pad(substr($Befehle[$i],1), 3, '0', STR_PAD_LEFT ),"    ",5);
        ...
        // +++ $funktionen->log_schreiben("Befehlsausführung OK. #".$InverterAdresse."L ".str_pad(substr($Befehle[$i],1), 3, '0', STR_PAD_LEFT ),"    ",8);
...
$funktionen->log_schreiben("---------   Stop   aec_wechselrichter.php    ----------------- ","|--",6);

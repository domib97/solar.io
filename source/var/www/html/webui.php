...
if (isset($_GET['config'])) {
  // Das muss bei einer noch eingeschaltet werden, wenn mehrere Wallboxen an einem 
  // Raspberry gesteuert werden sollen.
  // $Measurement = "Wallbox".$_GET['config'];
}
...
log_schreiben("Variable: ".$Steuerung,"    ",2);
  ...
  switch ($Steuerung) {
    ...
    case "WallboxSteuerung0":
      ...
      if (isset($_GET['config'])) {
        ...
        fwrite($fh,"stop");   // Nur bei der openWB nötig
        fclose($fh);
      }
      $query = $Measurement." wbSteuerung1=0,wbSteuerung2=0,wbSteuerung3=0";
    break;
    case "WallboxSteuerung1":
      $query = $Measurement." wbSteuerung1=1,wbSteuerung2=0,wbSteuerung3=0";
    break;
    case "WallboxSteuerung2":
      $query = $Measurement." wbSteuerung1=0,wbSteuerung2=1,wbSteuerung3=0";
    break;
    case "WallboxSteuerung3":
      $query = $Measurement." wbSteuerung1=0,wbSteuerung2=0,wbSteuerung3=1";
    break;
    default:
      $query = $Measurement." wbSteuerung1=0,wbSteuerung2=0,wbSteuerung3=0";
    break;
...
if (isset($_GET['uid'])) {
  log_schreiben("Variable: uid: ".$_GET['uid'],"    ",3);
  header("Location:http://solaranzeige.local:3000/d/".$_GET["uid"]);
  exit;
}
header('Location:http://solaranzeige.local:3000');
exit;

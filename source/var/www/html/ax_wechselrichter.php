...
$funktionen->log_schreiben("-----------   Start  ax_wechselrichter.php   ----------------- ","|--",6);
  ...
  for ($i = 0; $i < count($Befehle); $i++) {
    ...
    **************************************************************************/
    if (file_exists($Pfad."/befehle.ini.php")) {
      ...
      $funktionen->log_schreiben("Befehlsliste: ".print_r($Regler7,1),"|- ",10);
...
do {
  ...
  if ($Wert === false) {
    $funktionen->log_schreiben("Datenübertragung vom Wechselrichter war erfolglos! [Modus]\n","    ",5);
    $rc = "";
    if ($i < 5) {
        $i++;
        continue;
  ...
  if ($Wert === false) {
    $funktionen->log_schreiben("Datenübertragung vom Wechselrichter war erfolglos! [Warnungen]\n","    ",6);
    $rc = "";
    if ($i < 5) {
        $i++;
        continue;
  ...
  if (1 == 1) {
    ...
    $funktionen->log_schreiben(substr($Antwort,1,98)."  i: ".$k,"    ",9);
  ...
  $funktionen->log_schreiben(substr($Antwort,1,106)."  i: ".$k,"    ",6);
  ...
  if ($Wert === true and strlen($Antwort) > 106) {
    ...
    if ($aktuelleDaten["Set_Machine_type"] == "11" or $aktuelleDaten["Set_Machine_type"] == "20") {
      ...
      $funktionen->log_schreiben(substr($Antwort,1,106)."  i: ".$k,"    ",8);
    ...
    **************************************************************************/
    ...
    if ($aktuelleDaten["Modus"] == "F") {
      ...
      $funktionen->log_schreiben($aktuelleDaten["Fehlermeldung"],"** ",1);
...
$funktionen->log_schreiben("---------   Stop   ax_wechselrichter.php    ------------------ ","|--",6);
